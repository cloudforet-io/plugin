...
class PluginRefManager(BaseManager):
    def __init__(self, *args, **kwargs):
        ...
        self._installed_plugin_ref_model: InstalledPluginRef = self.locator.get_model(
            "InstalledPluginRef"
        )
    ...
    def create(self, params):
        ...
        _LOGGER.debug(f"[create] result : {plugin.to_dict()}")
        return plugin
    ...
    def get(self, supervisor_id, domain_id, plugin_id, version):
        ...
        plugin = self._installed_plugin_ref_model.get(
    ...
    def search_plugin(self, supervisor_id, plugin_id, version, domain_id):
        ...
        try:
            ...
            if total_count == 1:
                _LOGGER.debug(f"XXXXX Plugin.state = {plugins[0].plugin_owner.state}")
                return plugins[0]
            _LOGGER.debug(
    ...
    def install_plugin(self, supervisor, installed_plugin, params):
        ...
        try:
            ...
            installed_plugin_ref = self.get(
                supervisor_id, domain_id, plugin_id, version
            )
        ...
        _LOGGER.debug(
            f"[install_plugin] installed_plugin: {installed_plugin.plugin_id}"
        )
